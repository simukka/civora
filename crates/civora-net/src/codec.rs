//! Length-framed request-response codec for `/civora/sync/1`.
//!
//! Frames are `len (u32 LE) || payload`, with the payload decoded by the
//! canonical [`wire`] codecs. Reads enforce the wire size caps before
//! allocating, so a malicious peer cannot ask us to buffer more than
